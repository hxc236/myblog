...
class AdminAuthFlowTest {
    ...
    void adminApiRejectsLegacyJwt() throws Exception {
        // 手工构造 legacy JWT 形状的不透明字符串（#31 起构建不再含 jjwt）：
        // 任何非不透明会话令牌（含 legacy JWT）都必须被拒绝，哈希校验不匹配。
        Base64.Encoder b64 = Base64.getUrlEncoder().withoutPadding();
        String legacyJwt = b64.encodeToString(
                        "{\"alg\":\"HS256\"}".getBytes(StandardCharsets.UTF_8))
                + "." + b64.encodeToString(
                        "{\"sub\":\"legacy-user\"}".getBytes(StandardCharsets.UTF_8))
